...
#===============================================================================
def game_over():
    infofont = ("sans", 20, "bold")
    scr.create_text(map_to_ui_x(BOARD_WIDTH) / 2, map_to_ui_y(BOARD_HEIGHT) / 2, text = "GAME OVER | Score : %i" % get_score(), font = infofont, fill = TEXT_COLOR)
...
main = Tkinter.Tk()
playframe = Tkinter.Frame(main)
scr = Tkinter.Canvas(playframe, width=map_to_ui_x(BOARD_WIDTH), height=map_to_ui_y(BOARD_HEIGHT), bg=BACKGROUND_COLOR)
scr.pack()
restartb = Tkinter.Button(playframe, text = "Restart", command = init_tetris)
restartb.pack(fill = "x")
scorel = Tkinter.Label(playframe, text = "Score : 0")
scorel.pack(fill = "x")
playframe.pack()
main.protocol("WM_DELETE_WINDOW", quitw)
